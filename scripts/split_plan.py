...
p = pathlib.Path(sys.argv[1]); plan = yaml.safe_load(p.read_text())
...
for t in plan.get("tasks", []):
    ...
    (task_dir/'BRIEF.md').write_text(textwrap.dedent(f"""\
        # {tid}: {title}
        Lane: {lane}
    ...
    (task_dir/'.cursor.rules.md').write_text(textwrap.dedent(f"""\

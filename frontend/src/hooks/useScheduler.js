...
const schedulerApi = {
  ...
  getJobRuns: async (jobId, params = {}) => {
    ...
    const response = await api.get(`/scheduler/${jobId}/runs?${searchParams.toString()}`)
    return response.data
  },
...
export function useDeleteSchedule() {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      queryClient.removeQueries({ queryKey: SCHEDULER_KEYS.schedule(variables) })
    },
  })
}
...
export function useRunNow() {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: SCHEDULER_KEYS.schedule(variables.id) })
      // Invalidate runs for this job
      queryClient.invalidateQueries({ queryKey: SCHEDULER_KEYS.runs(variables.id) })
    },
  })
}
...
export function useSchedulerStats() {
  ...
  const stats = schedules.jobs?.reduce((acc, schedule) => {
    acc.total++

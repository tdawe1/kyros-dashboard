...
describe('VariantCard', () => {
  ...
  test('renders variant information correctly', () => {
    ...
    expect(screen.getByText('This is a test variant for LinkedIn')).toBeInTheDocument()
  })
  ...
  test('shows correct readability color for different levels', () => {
    const { rerender } = render(
      <VariantCard {...defaultProps} variant={{ ...mockVariant, readability: 'Excellent' }} />
    )
    expect(screen.getByText('Excellent')).toHaveClass('text-green-400')

    rerender(<VariantCard {...defaultProps} variant={{ ...mockVariant, readability: 'Poor' }} />)
